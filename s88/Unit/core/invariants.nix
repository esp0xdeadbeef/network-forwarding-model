...
let
  ...
  validateEndpoint =
    ...
    if need4 && (addr4 == null || peerAddr4 == null) then
      throw ''
        network-forwarding-model: incomplete IPv4 core uplink endpoint
      ...
      throw ''
        network-forwarding-model: incomplete IPv6 core uplink endpoint
...
in
{
  ...
  requireUniqueForwardingNames =
    ...
    if duplicateNames == [ ] then
      ...
      throw ''
        network-forwarding-model: forwarding uplink names must be unique across core units
