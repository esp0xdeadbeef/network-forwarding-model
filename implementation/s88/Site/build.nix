...
let
  ...
  inputViewMod = import ./build/input-view.nix {
    inherit
      lib
      utils
      trace
  ...
  inputView = inputViewMod { inherit enterprise siteId site; };
  ...
  canonicalOrdering = trace.emit "site:${enterprise}.${siteId}:canonical-ordering" (
    transitOrderingMod.canonicalize {
  ...
  rolesResult = trace.emit "site:${enterprise}.${siteId}:roles" (
    rolesMod.compute {
      inherit
        lib
        site
  ...
  wanResult = trace.emit "site:${enterprise}.${siteId}:wan" (
    wanMod.build {
      inherit
        lib
        site
  ...
  enforcementResult = trace.emit "site:${enterprise}.${siteId}:enforcement" (
    enfMod.build {
      inherit
        lib
        site
  ...
  topologyResult = trace.emit "site:${enterprise}.${siteId}:topology" (
    topoMod.build {
      inherit
        lib
        site
