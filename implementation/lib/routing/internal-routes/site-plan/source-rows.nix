...

{
  build =
    ...
    let
      ...
      entriesByKind = map enrichEntry (
        (if includeP2p then remotePrefixFacts.p2pEntries else [ ])
        ++ (
          if includeTenant then
            map normalizeTenantEntry (remotePrefixFacts.tenantOwnerEntries or [ ])
          else
            [ ]
        )
      ...
      resolutionKey =
        ...
        in
        "${nodeName}|${toString (e.owner or "")}|${toString (e.kind or "")}|${toString (e.overlay or "")}|${toString (e.peerSite or "")}|${
      ...
      appendScopedRows =
        ...
        builtins.foldl' (acc: nodeName: appendEntry acc nodeName entry) groups eligibleNodeNames;
      ...
      addEntryGroups =
        ...
        in
        builtins.foldl' (
          acc: scope: appendScopedRows acc eligibleNodeNames (entry // { routeScope = scope; })
        ) groupedBase serviceScopes;

      remoteGroups = builtins.foldl' addEntryGroups { } entriesByKind;
      remoteGroupValues = builtins.attrValues remoteGroups;
      eligiblePairCount = builtins.foldl' (
        acc: group: acc + builtins.length (group.entries or [ ])
      ) 0 remoteGroupValues;
      rejectedPairCount = (builtins.length entriesByKind * builtins.length nodeNames) - eligiblePairCount;
      classCounts = builtins.mapAttrs (_: records: builtins.length records) (
