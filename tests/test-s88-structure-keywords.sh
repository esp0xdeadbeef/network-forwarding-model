#!/usr/bin/env bash
set -euo pipefail
# GAMP-ID: RTM-GUARD-NFM-S88-KEYWORDS-001
...
warning_file="${tmp_dir}/s88-keyword-warnings.tsv"
: >"${matches_file}"
: >"${warning_file}"
...
scan_group() {
  local group="$1"
  local pattern="$2"
  local output_file="${3:-${matches_file}}"
  ...
  rg \
    ...
    "${repo_root}/s88" \
    "${repo_root}/lib" \
    "${repo_root}/implementation/s88" \
    "${repo_root}/implementation/lib" \
    2>/dev/null \
...
scan_group \
  ...
  '\b(esp0xdeadbeef|enterpriseA|enterpriseB|espbranch|site-a|site-b|site-c|s-router|b-router|c-router|hetzner|nebula|s-sigma|lab-s-sigma)\b'
...
fail "Implement the S88-style structure before this test may pass; generated name parsing and concrete lab identities may not be hidden behind wrappers."
fail "Domain words are scanned as coverage warnings, but this hard failure is reserved for parser primitives and generated-name parsing that rederive model structure from strings."
fail "This is a hard failure because scattered string parsing can miss compiled output families, lanes, and policy rows when the model shape changes."
