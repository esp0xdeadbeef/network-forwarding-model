...
# FS-470: an overlay's underlay access is directly linked to the overlay's
...
jq -e --arg a "${access_lane}" --arg c "${core_lane}" '
  ...
  | ($i[$a].routes.ipv4 | any(.dst == "10.10.0.0/24"))
    and ($i[$c].routes.ipv4 | all(.dst != "10.10.0.0/24"))
' "${model_json}" >/dev/null

pass_timed "tenant-prefix-routes-via-access-not-overlay-core"
