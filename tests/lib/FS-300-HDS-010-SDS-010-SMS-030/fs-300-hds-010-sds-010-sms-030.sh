#!/usr/bin/env bash
set -euo pipefail
# GAMP-ID: FS-300-HDS-010-SDS-010-SMS-030
...
pass_timed "fs300-ineligible-lane-route-rejection:compile" "${start_ms}"
...
' "${output_json}" >/dev/null || {
  echo "FAIL fs300-ineligible-lane-route-rejection: ineligible access lanes must not receive east-west overlay route candidates or destination-only fallback routes" >&2
  jq '
...
pass_timed "fs300-ineligible-lane-route-rejection"
