...

{
  build =
    ...
    let
      ...
      duplicateForwardingUplinkNames =
        lib.filter (
          uplinkName:
          builtins.length (lib.filter (entry: entry.name == uplinkName) uplinkNameEntries) > 1
        ) (lib.unique (map (entry: entry.name) uplinkNameEntries));

      _uniqueForwardingUplinkNames =
        if duplicateForwardingUplinkNames == [ ] then
          true
        else
          throw ''
            network-forwarding-model: forwarding uplink names must be unique across cores

            duplicate uplink names: ${lib.concatStringsSep ", " duplicateForwardingUplinkNames}
          '';

      _completeUplinkEndpoints =
        builtins.seq _uniqueForwardingUplinkNames (
          builtins.foldl' (
            acc: spec:
            builtins.seq acc (validateUplinkEndpoint spec.core spec.uplink)
          ) true wanSpecs
        );
    ...
    builtins.seq _haveCore (
      builtins.seq _haveUplinkCore (
        builtins.seq _completeUplinkEndpoints {
